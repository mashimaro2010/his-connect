...
export class HisPmkModel {
    ...
    getWard(db: Knex, wardCode: string = '', wardName: string = '') {
            ...
            // ����ա���к� wardCode ����������͹� WHERE
            if (wardCode) {
                query = query.where('WARDCODE', wardCode); // ����Ԫ��ͤ�������������촤�� 'wardcode'
            }
    
            // ����ա���к� wardName ����������͹� WHERE (�� LIKE ���ͤ��Һҧ��ǹ)
            if (wardName) {
                query = query.where('WARDNAME', 'like', `%${wardName}%`); // ����Ԫ��ͤ������������촤�� 'wardname'
            }
        ...
        countBedNo(db: Knex) {
            ...
            return db('ERP_GETBEDNO')
                ...
                .first()
                .then((result) => {
                    // Knex �ѡ�Ф׹��Ҩӹǹ�� string, ����ŧ�� number
                    return { total_bed: parseInt(result.total_bed as string) || 0 };
                })
                .catch(() => {
                    // �ó��Դ error ���׹��� 0
                    return { total_bed: 0 };
        ...
        async getBedNo(db: Knex, bedno: any = null, start = -1, limit: number = 1000) {
            let query = db('ERP_GETBEDNO')
                .select('*');
    
            // 1. ���͹䢡�á�ͧ��������Ţ��§ (����ա���к�)
            if (bedno) {
                query = query.where('BEDNO', bedno);
            }
    ...
    let query = db('ERP_ADMIT')
        .leftJoin(
            'ERP_CONCURRENTIPDBYWARD',
            'ERP_ADMIT.WARD_CODE',
            'ERP_CONCURRENTIPDBYWARD.WARDCODE'
        )
        .select(
            'ERP_CONCURRENTIPDBYWARD.WARDCODE as wardcode',
            'ERP_CONCURRENTIPDBYWARD.WARDNAME as wardname',
            'ERP_CONCURRENTIPDBYWARD.STD_CODE as std_code',
            'ERP_CONCURRENTIPDBYWARD.DATE_MODIFIED as date_modified',

            // bed capacity จาก view
            'ERP_CONCURRENTIPDBYWARD.NORMAL as bed_normal',
            'ERP_CONCURRENTIPDBYWARD.STROKE as bed_stroke',
            'ERP_CONCURRENTIPDBYWARD.SPECIAL as bed_special',
            'ERP_CONCURRENTIPDBYWARD.ICU as bed_icu',

            // ผู้ป่วยคงค้าง ณ เวลานั้น
            db.raw('COUNT(ERP_ADMIT.AN) as cases')
        )
        .whereRaw(whereRaw, [dateStart, dateEnd])
        .groupBy(
            'ERP_CONCURRENTIPDBYWARD.WARDCODE',
            'ERP_CONCURRENTIPDBYWARD.WARDNAME',
            'ERP_CONCURRENTIPDBYWARD.STD_CODE',
            'ERP_CONCURRENTIPDBYWARD.DATE_MODIFIED',
            'ERP_CONCURRENTIPDBYWARD.NORMAL',
            'ERP_CONCURRENTIPDBYWARD.STROKE',
            'ERP_CONCURRENTIPDBYWARD.SPECIAL',
            'ERP_CONCURRENTIPDBYWARD.ICU'
        )
        ...
        async concurrentIPDByClinic(db: Knex, date: any, limit = maxLimit) {
            const base = moment(date || undefined).isValid()
                ? moment(date)
                : moment();
        ...
        async sumOpdVisitByClinic(db: Knex, date: any, limit = maxLimit) {
    ...
    return db('ERP_OPDVISITBYCLINIC')
        .select(
            db.raw(`'EA0010714' as hospcode`),
            db.raw(`TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS') as date`, [dateStart]),
            'CLINICCODE as cliniccode',
            db.raw('SUM(CASES) as cases'),
            db.raw('SUM(ADMIT) as admit')
        )
        .whereRaw(
            `
            OPD_DATETIME BETWEEN
        ...
        async getVisitForMophAlert(db: Knex, date: any, limit = maxLimit) {
            const base = moment(date || undefined).isValid()
                ? moment(date)
                : moment();
